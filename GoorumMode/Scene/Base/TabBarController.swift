...
final class TabBarController: UITabBarController {
    ...
    private let moodVC = {
        ...
        vc.tabBarItem = UITabBarItem(title: "기록", image: Constants.IconImage.list, tag: 0)
        return nav
    }()

    private let chartVC = {
        let vc = ChartViewController()
        vc.tabBarItem = UITabBarItem(title: "차트", image: Constants.IconImage.stats, tag: 1)
        return vc
    }()
    ...
    private let settingVC = {
        ...
        let nav = UINavigationController(rootViewController: vc)
        vc.tabBarItem = UITabBarItem(title: "설정", image: Constants.IconImage.setting, tag: 2)
        return nav
    }()
